...
const {sealWorkflowPayload}=require('./secure-workflow-payload');
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const securePayload=sealWorkflowPayload({source_url:sourceUrl,source_kind:sourceKind,source_mime:sourceMime,source_name:sourceName,user_context:userContext,doodle_style:doodleStyle,plan_key:planKey},requestId);
    await gh(`/repos/${repo}/actions/workflows/analyze-illustrated.yml/dispatches`,{method:'POST',body:JSON.stringify({ref:'main',inputs:{request_id:requestId,secure_payload:securePayload}})});
