...
const PREVIEWS = {
  biblical: {url:'https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?auto=format&fit=crop&w=700&h=1244&q=85',alt:'Paisagem desértica de aparência antiga para histórias bíblicas'},
  devotional: {url:'https://images.unsplash.com/photo-1777421389422-519764272b2f?auto=format&fit=crop&w=700&h=1244&q=85',alt:'Bíblia aberta iluminada pela luz do sol para devocional'},
...
module.exports=async function handler(req,res){if(req.method!=='GET')return res.status(405).json({error:'Use GET.'});if(!requireAuth(req,res))return;const preset=String(req.query?.preset||'custom'),item=PREVIEWS[preset]||PREVIEWS.custom;res.setHeader('Cache-Control','private, max-age=86400');return res.status(200).json({...item,preset})};