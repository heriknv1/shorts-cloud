(()=>{
  ...
  const SPEED_LABELS={default:'Padrão',fast:'Rápido 1,05x',veryfast:'Veloz 1,10x'};
  ...
  function configureSpeed(){const speed=document.getElementById('voiceSpeed');if(!speed)return;const previous=['default','fast','veryfast'].includes(speed.value)?speed.value:'default';speed.innerHTML='<option value="default">Padrão</option><option value="fast">Rápido — 1,05x</option><option value="veryfast">Veloz — 1,10x</option>';speed.value=previous;const label=speed.parentElement?.querySelector('label');if(label)label.textContent='Velocidade da narração'}
