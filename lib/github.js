...
// Logical creation-history reset requested by the owner. Older runs remain recoverable on GitHub but are hidden and do not consume quota.
const QUOTA_RESET_AT=Date.parse('2026-08-31T23:37:54.000Z');
...
async function ghBuffer(path){const{token}=config();const response=await fetch(`${API}${path}`,{headers:{Accept:'application/vnd.github+json',Authorization:`Bearer ${token}`,'X-GitHub-Api-Version':'2022-11-28'}});if(!response.ok)throw new Error(`GitHub HTTP ${response.status}`);return Buffer.from(await response.arrayBuffer())}
...
function consumesQuota(run){if(run?.name!=='Generate Short'||!afterQuotaReset(run)||run.head_branch!==STUDIO_BRANCH)return false;if(run.status!=='completed')return true;return run.conclusion==='success'}
...
module.exports={gh,ghBuffer,config,saoPauloDate,workflowRuns,usedToday,consumesQuota,afterQuotaReset,STUDIO_BRANCH,QUOTA_RESET_AT};
