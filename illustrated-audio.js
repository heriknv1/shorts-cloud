...
-----END ${descriptor}-----`;
  ...
  (() => {
    ...
    function sourceMode() {
      return $("illustratedSourceMode")?.value === "link" ? "link" : "file";
    }
    function contextValue() {
      return String($("illustratedContext")?.value || "").trim().slice(0, 600);
    }
    function doodleStyle() {
      return $("illustratedDoodleStyle")?.value || "clean-doodle";
    }
    function linkValue() {
      return String($("illustratedSourceLink")?.value || "").trim();
    }
    function sourceUrl() {
      return sourceMode() === "link" ? linkValue() : "";
    }
    function sourcePathname() {
      return sourceMode() === "file" ? state.uploadPath : "";
    }
    function analysisSignature() {
      return JSON.stringify([sourceMode(), sourceUrl(), sourcePathname(), contextValue(), doodleStyle()]);
    }
    function safeName(value) {
      return String(value || "conteudo").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^A-Za-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 90) || "conteudo";
    }
    ...
    function setSourceStatus(text, kind = "", progress = null) {
      const el = $(sourceMode() === "link" ? "illustratedLinkStatus" : "illustratedSourceStatus");
      if (el) {
        el.textContent = text;
        el.className = `illustrated-source-status ${kind}`.trim();
      }
      const bar = $("illustratedUploadBar");
      if (bar && progress !== null) bar.style.width = `${Math.max(0, Math.min(100, Number(progress) || 0))}%`;
    }
    function rightsConfirmed() {
      return !!$("illustratedRights")?.checked;
    }
    function savePending(signature) {
      try {
        sessionStorage.setItem(PENDING_KEY, JSON.stringify({ at: Date.now(), analysisId: state.analysisId, analysisKey: state.analysisKey, signature, mode: sourceMode(), uploadPath: state.uploadPath, link: linkValue(), mime: state.mime, name: state.name, size: state.size, context: contextValue(), style: doodleStyle() }));
      } catch {
      }
    }
    function clearPending(reset = false) {
      try {
    ...
    function injectStyles() {
      ...
      style.id = "illustratedAudioStyles";
      style.textContent = `
    ...
    function injectUI() {
      const grid = $("presetGrid"), topic = $("topic");
      if (!grid || !topic || $("illustratedSourcePanel")) return;
      const button = document.createElement("button");
      button.type = "button";
      button.className = "preset";
      button.dataset.preset = "audio-illustrated";
      button.innerHTML = "<strong>\xC1udio Ilustrado</strong><span>\xC1udio, v\xEDdeo ou link \u2022 desenho divertido sincronizado</span>";
      const life = grid.querySelector('[data-preset="life-lessons"]');
      if (life) life.insertAdjacentElement("afterend", button);
      else grid.appendChild(button);
      const panel = document.createElement("div");
      panel.id = "illustratedSourcePanel";
      panel.className = "illustrated-source-panel";
      panel.hidden = true;
      panel.innerHTML = `
      <div class="illustrated-source-head"><span class="illustrated-source-icon">\u270E</span><div><h3>Transforme um conte\xFAdo real em desenho</h3><p>O \xE1udio original conduz as cenas. A an\xE1lise identifica falas, participantes, a\xE7\xF5es, mudan\xE7as de contexto e oportunidades de humor.</p></div><button type="button" class="ghost" id="cancelIllustratedAnalysis" hidden>Cancelar an\xE1lise</button></div>
      <div class="illustrated-source-grid">
        <div><label for="illustratedSourceMode">Entrada</label><select id="illustratedSourceMode"><option value="file" selected>Enviar \xE1udio ou v\xEDdeo</option><option value="link">Colar link p\xFAblico</option></select></div>
        <div><label for="illustratedDoodleStyle">Tra\xE7o</label><select id="illustratedDoodleStyle"><option value="clean-doodle" selected>Tra\xE7o limpo e divertido</option><option value="soft-accent">Preto com cores suaves</option><option value="playful-ink">Nanquin expressivo</option></select></div>
        <div class="wide" id="illustratedFileWrap"><div class="illustrated-file-box"><div class="illustrated-file-actions"><button type="button" class="ghost" id="chooseIllustratedFile">Escolher \xE1udio ou v\xEDdeo</button><span class="illustrated-file-name" id="illustratedFileName">Nenhum arquivo selecionado</span></div><input id="illustratedFileInput" type="file" accept="audio/*,video/*" hidden><div class="illustrated-upload-track"><div class="illustrated-upload-bar" id="illustratedUploadBar"></div></div><div class="illustrated-source-status" id="illustratedSourceStatus">Formatos comuns de \xE1udio e v\xEDdeo, com at\xE9 200 MB.</div><div id="illustratedMediaPreview"></div></div></div>
        <div class="wide" id="illustratedLinkWrap" hidden><label for="illustratedSourceLink">Link do v\xEDdeo ou \xE1udio</label><input id="illustratedSourceLink" type="url" inputmode="url" placeholder="https://..."><div class="muted">O conte\xFAdo precisa estar p\xFAblico. Se a plataforma bloquear a leitura, envie o arquivo diretamente.</div><div class="illustrated-source-status" id="illustratedLinkStatus">Cole um endere\xE7o p\xFAblico iniciado por https://.</div></div>
        <div class="wide"><label for="illustratedContext">Orienta\xE7\xE3o adicional <span class="muted">(opcional)</span></label><textarea id="illustratedContext" rows="2" maxlength="600" placeholder="Ex.: mantenha a conversa familiar e destaque a rea\xE7\xE3o engra\xE7ada no final"></textarea></div>
      </div>
      <label class="illustrated-rights"><input id="illustratedRights" type="checkbox"><span>Confirmo que tenho autoriza\xE7\xE3o para utilizar o \xE1udio ou v\xEDdeo enviado.</span></label>`;
      ...
      $("illustratedSourceLink").addEventListener("input", () => {
        state.planSignature = "";
        if (validHttps(linkValue())) setSourceStatus("Link pronto para an\xE1lise.", "ok");
        else setSourceStatus("Cole um endere\xE7o p\xFAblico iniciado por https://.", "");
      });
      $("illustratedContext").addEventListener("input", () => {
        state.planSignature = "";
      });
      $("illustratedDoodleStyle").addEventListener("change", () => {
        state.planSignature = "";
      });
    ...
    function syncSourceMode() {
      ...
      if ($("illustratedFileWrap")) $("illustratedFileWrap").hidden = link;
      if ($("illustratedLinkWrap")) $("illustratedLinkWrap").hidden = !link;
      if (link) setSourceStatus(validHttps(linkValue()) ? "Link pronto para an\xE1lise." : "Cole um endere\xE7o p\xFAblico iniciado por https://.", validHttps(linkValue()) ? "ok" : "");
      else if (state.uploadPath) setSourceStatus("Arquivo enviado e pronto para an\xE1lise.", "ok", 100);
    }
    function renderLocalPreview(file) {
      const wrap3 = $("illustratedMediaPreview");
      if (!wrap3) return;
      if (state.fileObjectUrl) URL.revokeObjectURL(state.fileObjectUrl);
      state.fileObjectUrl = URL.createObjectURL(file);
      wrap3.innerHTML = "";
      const media = document.createElement(file.type.startsWith("video/") ? "video" : "audio");
      media.className = "illustrated-media-preview";
      media.controls = true;
      media.preload = "metadata";
      if (media.tagName === "VIDEO") media.muted = true;
      media.src = state.fileObjectUrl;
      wrap3.appendChild(media);
    }
    async function prepareFile(file) {
      if (!file) return;
      if (!/^(audio|video)\//i.test(file.type)) {
        setSourceStatus("Escolha um arquivo de \xE1udio ou v\xEDdeo v\xE1lido.", "bad");
        return;
      }
      if (file.size > MAX_SOURCE_BYTES) {
        setSourceStatus("O arquivo ultrapassa o limite de 200 MB.", "bad");
        return;
      }
      state.file = file;
      state.mime = file.type || "application/octet-stream";
      state.name = file.name;
      state.size = file.size;
      state.uploadPath = "";
      state.planSignature = "";
      renderLocalPreview(file);
      $("illustratedFileName").textContent = `${file.name} \u2022 ${(file.size / 1024 / 1024).toFixed(1)} MB`;
      await uploadSource(file);
    }
    async function uploadSource(file) {
      state.uploading = true;
      $("chooseIllustratedFile").disabled = true;
    ...
    function syncPreview() {
      ...
      if (photo) {
        const src = previewSvg();
        if (photo.hidden) photo.hidden = false;
        if (photo.getAttribute("src") !== src) photo.src = src;
        photo.alt = "Pr\xE9via de anima\xE7\xE3o ilustrada minimalista";
      }
      if (fallback && !fallback.hidden) fallback.hidden = true;
      if ($("previewModeBadge")) $("previewModeBadge").textContent = "\xC1UDIO ILUSTRADO \u2022 DESENHO";
      if ($("mediaChip")) $("mediaChip").textContent = "\xC1udio original";
      if ($("previewCaption")) $("previewCaption").textContent = "As falas aparecem sincronizadas embaixo";
      if ($("visualNotice")) $("visualNotice").textContent = "Personagens consistentes, tra\xE7o simples e cada desenho ligado diretamente ao contexto da fala.";
      if ($("audioNotice")) $("audioNotice").textContent = "O \xE1udio original ser\xE1 preservado. N\xE3o ser\xE1 criada uma nova narra\xE7\xE3o nem m\xFAsica de fundo.";
      if ($("creativeNotice")) $("creativeNotice").textContent = "Cortes sincronizados, rea\xE7\xF5es expressivas e piadas visuais espec\xEDficas \u2014 sem movimentos de c\xE2mera ou cenas gen\xE9ricas.";
    }
    function rememberAndApplyDefaults() {
      const ids = ["visualStyle", "mediaMode", "cartoonStyle", "tone", "voice", "captions", "captionFont", "captionSize", "music", "editingPace", "sfxMode", "ambienceMode", "cleanExport", "brandingMode"];
      state.snapshot = { topicValue: $("topic")?.value || "", values: Object.fromEntries(ids.map((id) => [id, $(id)?.value])), durationDisabled: $("duration")?.disabled || false, durationLabel: $("duration")?.parentElement?.querySelector("label")?.textContent || "Dura\xE7\xE3o", stageTitle: document.querySelector('[data-stage="1"] h2')?.textContent || "", stageTip: document.querySelector('[data-stage="1"] .stage-tip')?.textContent || "", planText: $("planBtn")?.textContent || "" };
      ...
      if (duration) {
        ...
        const label = duration.parentElement?.querySelector("label");
        if (label) label.textContent = "Dura\xE7\xE3o \u2014 autom\xE1tica pelo conte\xFAdo";
      }
    }
    function hideBaseTopic(hidden) {
      const topic = $("topic"), label = topic?.previousElementSibling;
      if (topic) topic.classList.toggle("illustrated-base-field-hidden", hidden);
      if (label?.tagName === "LABEL") label.classList.toggle("illustrated-base-field-hidden", hidden);
    }
    function enableMode() {
      if (state.active) return;
      state.active = true;
      state.pollToken++;
      rememberAndApplyDefaults();
      document.body.dataset.illustratedMode = "on";
      document.querySelectorAll(".preset").forEach((b) => b.classList.toggle("active", b.dataset.preset === "audio-illustrated"));
      if ($("illustratedSourcePanel")) $("illustratedSourcePanel").hidden = false;
      const topic = $("topic");
      if (topic) topic.value = "Conte\xFAdo enviado para anima\xE7\xE3o ilustrada";
      hideBaseTopic(true);
      const title = document.querySelector('[data-stage="1"] h2'), tip = document.querySelector('[data-stage="1"] .stage-tip');
      if (title) title.textContent = "Qual conte\xFAdo voc\xEA quer transformar em desenho?";
      if (tip) tip.textContent = "Envie um arquivo ou cole um link p\xFAblico.";
      if ($("planBtn")) $("planBtn").textContent = "Analisar conte\xFAdo e criar storyboard";
      if ($("presetStatus")) $("presetStatus").textContent = "\xC1udio Ilustrado selecionado. O \xE1udio original ser\xE1 entendido e transformado em cenas criativas e coerentes.";
      document.dispatchEvent(new CustomEvent("shortcloud:preset-selected", { detail: "audio-illustrated" }));
      syncSourceMode();
      setTimeout(syncPreview, 80);
    }
    function disableMode() {
      if (!state.active) return;
      state.active = false;
      state.pollToken++;
      delete document.body.dataset.illustratedMode;
      if ($("illustratedSourcePanel")) $("illustratedSourcePanel").hidden = true;
      hideBaseTopic(false);
      const snap = state.snapshot || {};
      if ($("topic")) $("topic").value = snap.topicValue || "";
      for (const [id, value] of Object.entries(snap.values || {})) {
        const el = $(id);
        if (el && value !== void 0) {
          el.value = value;
          el.dispatchEvent(new Event("change", { bubbles: true }));
      ...
      const duration = $("duration");
      if (duration) {
        duration.disabled = !!snap.durationDisabled;
        const label = duration.parentElement?.querySelector("label");
        if (label) label.textContent = snap.durationLabel || "Dura\xE7\xE3o";
      }
      const title = document.querySelector('[data-stage="1"] h2'), tip = document.querySelector('[data-stage="1"] .stage-tip');
      if (title && snap.stageTitle) title.textContent = snap.stageTitle;
      if (tip && snap.stageTip) tip.textContent = snap.stageTip;
      if ($("planBtn")) $("planBtn").textContent = snap.planText || "Criar roteiro e avan\xE7ar";
      state.snapshot = null;
    }
    function validateBeforeAnalysis() {
      if (!rightsConfirmed()) return "Confirme que voc\xEA tem autoriza\xE7\xE3o para utilizar esse conte\xFAdo.";
      if (sourceMode() === "file") {
        if (state.uploading) return "Aguarde o arquivo terminar de ser enviado.";
        if (!state.uploadPath) return "Escolha e envie um \xE1udio ou v\xEDdeo antes de continuar.";
      } else if (!validHttps(linkValue())) return "Cole um link p\xFAblico v\xE1lido iniciado por https://.";
      return "";
    }
    async function waitForAnalysis(analysisId, analysisKey, token) {
      let transientFailures = 0;
      for (let attempt = 0; attempt < 600; attempt++) {
        if (token !== state.pollToken || !state.active) throw Error("An\xE1lise interrompida.");
        await wait(attempt === 0 ? 1800 : 3e3);
        const message2 = $("message");
        let response, data;
        try {
          response = await nativeFetch(`/api/illustrated-status?id=${encodeURIComponent(analysisId)}`, { cache: "no-store", headers: { "X-Analysis-Key": analysisKey } });
          ...
          if (transientFailures <= 8) {
            if (message2) {
              ...
              message2.textContent = "A an\xE1lise continua. Reconectando ao acompanhamento automaticamente\u2026";
            }
            continue;
          }
          throw Error("A an\xE1lise continua, mas n\xE3o foi poss\xEDvel acompanhar o progresso agora.");
        }
        if (!response.ok) {
          if ((response.status === 429 || response.status >= 500) && ++transientFailures <= 8) {
            if (message2) {
              message2.hidden = false;
              message2.style.color = "";
              message2.textContent = "O processamento continua. Tentando acompanhar novamente\u2026";
            }
            continue;
          }
          throw Error(data.error || "N\xE3o foi poss\xEDvel acompanhar a an\xE1lise.");
        }
        transientFailures = 0;
        if (message2) {
          message2.hidden = false;
          message2.style.color = "";
          message2.textContent = data.stage || "Entendendo o \xE1udio, as cenas e o contexto\u2026";
        }
        if (data.ready && data.plan) return data.plan;
        if (data.failed) {
          clearPending(true);
          throw Error(data.error || "N\xE3o foi poss\xEDvel analisar esse conte\xFAdo.");
        }
      }
      throw Error("A an\xE1lise demorou mais que o esperado. Tente novamente.");
    }
    function setAnalysisBusy(value) {
      const button = $("cancelIllustratedAnalysis");
      if (button) {
        button.hidden = !value;
        button.disabled = false;
        button.textContent = "Cancelar an\xE1lise";
      }
    }
    async function cancelAnalysis() {
      const button = $("cancelIllustratedAnalysis");
      if (!state.analysisId || !button) return;
      button.disabled = true;
      button.textContent = "Cancelando\u2026";
      try {
        const response = await nativeFetch("/api/illustrated-cancel", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: state.analysisId }) }), data = await response.json();
        if (!response.ok) throw Error(data.error || "N\xE3o foi poss\xEDvel cancelar.");
        state.pollToken++;
        clearPending(true);
        ...
        if (message2) {
          message2.hidden = false;
          message2.style.color = "";
          message2.textContent = "An\xE1lise cancelada. Voc\xEA pode escolher outro conte\xFAdo.";
        }
      } catch (error) {
        button.disabled = false;
        button.textContent = "Cancelar an\xE1lise";
        setSourceStatus(error.message || "N\xE3o foi poss\xEDvel cancelar a an\xE1lise.", "bad");
      }
    }
    function enhanceStoryboard() {
      if (!state.active || !state.lastPlan?.scenes) return;
      document.querySelectorAll("#sceneEditor .scene").forEach((el, index) => {
        const scene = state.lastPlan.scenes[index];
        if (!scene) return;
        const label = el.querySelector(".n")?.parentElement?.querySelector("label");
        if (label) label.textContent = "Fala original";
        const narration = el.querySelector(".n");
        if (narration) {
          narration.readOnly = true;
          narration.classList.add("illustrated-readonly");
        }
        let meta = el.querySelector(".illustrated-meta");
        if (!meta) {
          meta = document.createElement("div");
          meta.className = "illustrated-meta";
          el.querySelector(".scene-head")?.insertAdjacentElement("afterend", meta);
        }
        const start = Number(scene.start || 0).toFixed(1), end = Number(scene.end || 0).toFixed(1), word = String(scene.on_screen_text || "").trim(), gag = String(scene.visual_gag || scene.visual_description || "").trim(), speaker = String(scene.speaker || "").trim();
        meta.innerHTML = `<strong>${start}s\u2013${end}s${speaker ? ` \u2022 ${escapeHtml(speaker)}` : ""}</strong>${word ? ` \u2022 Palavra visual: \u201C${escapeHtml(word)}\u201D` : ""}<br>${escapeHtml(gag)}`;
        const badge = el.querySelector(".badge");
        if (badge) badge.textContent = "desenho sincronizado";
      });
    }
    window.fetch = async function(input, init) {
      const url = typeof input === "string" ? input : input?.url || "";
      if (!state.active) return nativeFetch(input, init);
      if (init?.method === "POST" && url.includes("/api/scene-query")) {
        let body = {};
        try {
          body = JSON.parse(init.body || "{}");
        } catch {
        }
        const desc = String(body.visualDescription || body.narration || "").trim();
        return jsonResponse(200, { visual_query: desc, visual_query_backup: desc, recommended_media: "image" });
      }
      if (init?.method === "POST" && url.includes("/api/plan")) {
        const error = validateBeforeAnalysis();
        ...
        if (!(state.analysisId && /^[a-f0-9]{64}$/.test(state.analysisKey) && state.pendingSignature === signature)) {
          const started = await nativeFetch("/api/illustrated-plan", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }), startedData = await started.json();
          if (!started.ok) return jsonResponse(started.status, startedData);
          state.analysisId = startedData.analysisId;
          state.analysisKey = startedData.analysisKey;
          state.pendingSignature = signature;
          savePending(signature);
        }
        const token = ++state.pollToken;
        setAnalysisBusy(true);
        try {
          const plan = await waitForAnalysis(state.analysisId, state.analysisKey, token);
          clearPending(false);
          state.analysisKey = "";
          state.lastPlan = plan;
          state.planSignature = signature;
          state.duration = Number(plan?.source?.duration_seconds || plan?.duration_seconds || 0);
          setTimeout(enhanceStoryboard, 80);
          return jsonResponse(200, { plan, preset: "\xC1udio Ilustrado" });
        } catch (err) {
          return jsonResponse(500, { error: err.message || "N\xE3o foi poss\xEDvel analisar esse conte\xFAdo." });
        } finally {
          setAnalysisBusy(false);
        }
      }
      if (init?.method === "POST" && url.includes("/api/generate")) {
        const error = validateBeforeAnalysis();
        if (error) return jsonResponse(400, { error });
        if (!state.planSignature || state.planSignature !== analysisSignature()) return jsonResponse(400, { error: "O conte\xFAdo ou o estilo mudou depois da an\xE1lise. Crie o storyboard novamente para manter as cenas coerentes." });
        let body = {};
        try {
          body = JSON.parse(init.body || "{}");
        } catch {
        }
        body.presetKey = "audio-illustrated";
        body.topic = body.plan?.title || "\xC1udio Ilustrado";
        body.duration = state.duration || Number(body.plan?.source?.duration_seconds) || 30;
        body.visualStyle = "cartoon";
        body.cartoonStyle = "classic-2d";
        body.mediaMode = "photos";
        body.voice = "off";
        body.music = "off";
        body.editingPace = "fast";
        body.sfxMode = "off";
        body.ambienceMode = "off";
        body.sourceUrl = sourceUrl();
        body.sourcePathname = sourcePathname();
        body.sourceKind = sourceMode() === "link" ? "link" : state.mime.startsWith("audio/") ? "audio" : "video";
        body.sourceMime = sourceMode() === "link" ? "application/octet-stream" : state.mime;
        body.sourceName = sourceMode() === "link" ? safeName(new URL(linkValue()).hostname) : state.name;
        body.sourceAnalysisId = state.analysisId;
        body.doodleStyle = doodleStyle();
        body.sourceRights = true;
    ...
    function boot() {
      ...
      document.addEventListener("click", (event) => {
        ...
        if (!preset) return;
        if (preset.dataset.preset === "audio-illustrated") enableMode();
        else disableMode();
      }, { capture: true });
      const sceneEditor = $("sceneEditor");
      if (sceneEditor) new MutationObserver(() => setTimeout(enhanceStoryboard, 0)).observe(sceneEditor, { childList: true, subtree: true });
      const preview = $("previewMedia");
      if (preview) new MutationObserver(() => {
        if (state.active) setTimeout(syncPreview, 0);
      }).observe(preview, { attributes: true, subtree: true, attributeFilter: ["src", "hidden"] });
      try {
        const saved = JSON.parse(sessionStorage.getItem(PENDING_KEY) || "null");
        if (saved && Date.now() - Number(saved.at || 0) < 3 * 60 * 60 * 1e3 && /^ill-[0-9]{10,16}-[a-f0-9]{8}$/.test(saved.analysisId || "") && /^[a-f0-9]{64}$/.test(saved.analysisKey || "")) {
          state.analysisId = saved.analysisId;
          state.analysisKey = saved.analysisKey;
          state.pendingSignature = saved.signature || "";
          state.uploadPath = saved.uploadPath || "";
          state.mime = saved.mime || "";
          state.name = saved.name || "";
          state.size = Number(saved.size || 0);
          $("illustratedSourceMode").value = saved.mode === "link" ? "link" : "file";
          $("illustratedSourceLink").value = saved.link || "";
          $("illustratedContext").value = saved.context || "";
          $("illustratedDoodleStyle").value = saved.style || "clean-doodle";
          $("illustratedRights").checked = true;
          if (state.name) $("illustratedFileName").textContent = `${state.name} \u2022 arquivo tempor\xE1rio preservado`;
          enableMode();
          setAnalysisBusy(true);
          setTimeout(() => $("planBtn")?.click(), 300);
        } else if (saved) clearPending(true);
      } catch {
        clearPending(true);
      }
    }
