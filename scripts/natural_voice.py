...
def polish_voice(src,dst):
    filters='highpass=f=65,lowpass=f=13500,acompressor=threshold=-20dB:ratio=1.45:attack=30:release=260:makeup=1.12,alimiter=limit=0.95'
...
def edge_voice(spoken,idx,wav):
    _,edge_voice_name,_=selected_profile(); _,_,pitch,rate=voice_settings(); mp3=WORK/f'voice_edge_{idx:02d}.mp3'; rawwav=WORK/f'voice_edge_raw_{idx:02d}.wav'
    ...
    run(['ffmpeg','-y','-i',str(mp3),'-ar','48000','-ac','1','-c:a','pcm_s16le',str(rawwav)],quiet=True); polish_voice(rawwav,wav)
