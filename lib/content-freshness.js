...
function speedFactor(speed){return speed==='veryfast'?1.10:speed==='fast'?1.05:1}
function targetNarrationWords(duration,speed){
  const seconds=Math.min(70,Math.max(60,Number(duration)||65));
  return Math.round(seconds*2.42*speedFactor(speed));
}
function minNarrationWords(duration,speed){return Math.round(targetNarrationWords(duration,speed)*0.84)}
function maxNarrationWords(duration,speed){return Math.round(targetNarrationWords(duration,speed)*1.18)}
