...
const {gh,ghBuffer,config}=require('./github');
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(run.status==='completed'){
      ...
      if(artifact){
        const archive=await ghBuffer(`/repos/${repo}/actions/artifacts/${artifact.id}/zip`),files=unzipSync(new Uint8Array(archive)),errorEntry=Object.entries(files).find(([name])=>name.endsWith('illustrated_error.enc'));
        if(errorEntry){const raw=decryptPlan(Buffer.from(errorEntry[1]),id,analysisKey),failure=JSON.parse(raw),message=String(failure?.error||'A análise foi interrompida.').replace(/[\r\n]+/g,' ').slice(0,420);res.setHeader('Cache-Control','no-store');return res.status(200).json({ready:false,failed:true,error:message})}
        const entry=Object.entries(files).find(([name])=>name.endsWith('illustrated_plan.enc'));
        if(entry){const raw=decryptPlan(Buffer.from(entry[1]),id,analysisKey);if(raw.length>80000)throw new Error('plano grande demais');const plan=JSON.parse(raw);if(!Array.isArray(plan?.scenes)||plan.scenes.length<4)throw new Error('plano inválido');res.setHeader('Cache-Control','no-store');return res.status(200).json({ready:true,stage:'Storyboard criativo pronto.',plan})}
      }
      if(run.conclusion!=='success')return res.status(200).json({ready:false,failed:true,error:'A análise foi interrompida antes de concluir. Tente novamente; o conteúdo enviado não foi publicado.'});
      return res.status(200).json({ready:false,stage:'Finalizando o storyboard criativo…'});
    }
