(()=>{
  ...
  async function syncPreview(){
    ...
    const caption=$('previewCaption');if(caption)caption.textContent=isReal()?'Alguns dos relatos mais assustadores não nasceram da ficção.':'Às vezes, o pior não é o que você vê — é o que percebe tarde demais.';
    ...
    if(visual==='cartoon'){
      ...
      const base=isReal()?'documentary reconstruction of a documented historical mystery, eerie realistic location':'psychological horror suspense, lonely hallway at night, subtle unsettling detail';
      photo.src=`https://image.pollinations.ai/prompt/${encodeURIComponent(`${base}, low key lighting, volumetric shadows, ${styleText}, non graphic, vertical 9:16, no text, no watermark`)}?width=512&height=910&nologo=true&seed=${isReal()?1900:13082026}&enhance=true`;photo.alt=isReal()?'Prévia de conto real de terror':'Prévia de terror e suspense';return;
    }
  ...
  function inject(){
    ...
    grid.querySelectorAll('.preset:not([data-preset="horror"]):not([data-preset="horror-real"])').forEach(b=>b.addEventListener('click',()=>setMode('')));
  }
  function patchBody(body,url){
    if(!horrorMode)return body;
    try{const parsed=JSON.parse(body||'{}');parsed.presetKey=horrorMode;const sentinel=['Criar uma história original de terror e suspense','Criar um conto real de terror diferente'];if(sentinel.includes(parsed.topic)){parsed.topic=parsed.plan?.title||''}if((url.includes('/api/plan')||url.includes('horror-plan'))&&sentinel.includes(String(JSON.parse(body||'{}').topic||'')))parsed.topic='';return JSON.stringify(parsed)}catch{return body}
  }
